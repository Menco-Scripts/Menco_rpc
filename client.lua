local function formatString(template, variables)
    return (template:gsub("{(.-)}", function(key)
        return tostring(variables[key] or "nil")
    end))
end

local function getLocalizedString(path)
    local parts = {}
...
local function getPlayerInfo()
    ...
    if streetHash ~= 0 then
        ...
        local unknownText = getLocalizedString("status.unknown")
        if unknownText then
...
local function buildStatusMessage(playerInfo)
    if playerInfo.isInVehicle and playerInfo.vehicleName then
        local template = getLocalizedString("status.driving") or "Driving a {vehicle} on {street}"
        return formatString(template, {
            vehicle = playerInfo.vehicleName,
            street = playerInfo.streetName
        })
    else
        local template = getLocalizedString("status." .. playerInfo.movementState) or "{state} on {street}"
...
local function updateDiscordPresence()
    ...
    local playerIdText = formatString(getLocalizedString("actions.playerId") or "Player ID: {id}", {
...
RegisterCommand('discord', function()
    local discordLink = Config.DiscordLink or "https://discord.gg/unknown"
    local template = getLocalizedString("commands.discordMessage") or "Join our Discord server: {discord}"
    local msg = formatString(template, { discord = discordLink })
